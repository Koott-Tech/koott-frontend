  ...
  useEffect(() => {
    ...
    const normalizeImageUrl = (url) => {
      ...
      if (supabaseStorageMatch) {
        ...
        return `https://www.koott.in/api/images/${bucket}/${encodeURIComponent(filename)}`;
      }
      if (url.startsWith('/api/images/')) {
        return `https://www.koott.in${url}`;
      }
      return `https://www.koott.in${url.startsWith('/') ? url : `/${url}`}`;
    };
    ...
    const url = `https://www.koott.in/therapist-profile?doctor=${encodeURIComponent(doctorParam || selectedDoctor.id)}`;
